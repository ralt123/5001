...
def index(): 
     # get user id    
    user = session["user_id"]
    for_name = db.execute("SELECT username FROM users WHERE user_id = :user", user = user)
    ...
    return render_template("index.html", random_rows=random_rows, for_name=for_name)
...
def itm_page(item_id):
    ...
    # else if user reached route via GET (as by clicking a link or via redirect)
        ...
        seller_email = db.execute("SELECT email FROM users WHERE user_id = :seller_id", seller_id=seller_id)[0]["email"]
        seller_phonenumber = db.execute("SELECT phonenumber FROM users WHERE user_id = :seller_id", seller_id=seller_id)[0]["phonenumber"]
        return render_template("itm.html", item_row=item_row, watchlist_status=watchlist_status, seller_name=seller_name, item_status=item_status, seller_email=seller_email, seller_phonenumber=seller_phonenumber)
...
def login():
    ...
    if request.method == "POST":
        ...
        # it will show a welcom sign when you enter homepage
        flash("Welcome!")
    ...
    # else if user reached route via GET (as by clicking a link or via redirect)
        ...
        random_rows = db.execute("SELECT * FROM items WHERE status=:live LIMIT 9", live=1)
        return render_template("login.html", random_rows=random_rows)
...
def my_auctions():
    ...
    for item in items:
        ...
        if auction_status == 1:
            ...
            tempDict["timestamp"] = db.execute("SELECT timestamp FROM items WHERE item_id = :item_id", item_id = item)[0]["timestamp"]
            tempDict["username"] = db.execute("SELECT username FROM users WHERE user_id = :user", user = user)[0]["username"]
...
def register():
    ...
    if request.method == "POST":                                            # if user reached route via POST (as by submitting a form via POST)       
        if not request.form.get("username"):                                # ensure username was submitted
            ...
            return apology("must provide password")            
        if not request.form.get("email"):                                   # ensure email was submitted
            return apology("must provide email address")      
        if not request.form.get("phonenumber"):                             # ensure phone number was submitted
            return apology("must provide phonenumber") 
        phonenumber = request.form.get("phonenumber")                       #get phone number       
        if len(str(phonenumber)) >= 12:                                     #if phone number is either bigger or smaller then 11, then it return an apology
            return apology("phone number too long")
        elif len(str(phonenumber)) <= 10:
            return apology("phone number too short")           
        if not request.form.get("address"):                                 # ensure address was submitted
            return apology("must provide address")    
        address = request.form.get("address")                               #get address
        user_email = request.form.get("email")                              # get user email
        if not request.form.get("venmo"):                                   # ensure user insert Venmo
            return apology("must provide Venmo username")
        venmo_username = request.form.get("venmo")                          # get venmo
        venmo_username = venmo_username.replace('@', '');                   # remove "-" characters from input       
        ...
        if len(rows) == 1:                                                  # ensure username or email are already not being used
            return apology("username or email are already being used!")

        if request.form.get("password") != request.form.get("confirmation"):# ensure user input a password and the same password again
            return apology("passwords are not matching :(")

        # if everything is ok, register username and hashed password in our database
        user_id = db.execute("INSERT INTO users (username, hash, email, venmo, phonenumber, address) VALUES(:users, :hash, :email, :venmo, :number, :address)", users=request.form.get("username"), hash=pwd_context.encrypt(request.form.get("password")), email=user_email, venmo=venmo_username, phonenumber=phonenumber, address=address)

        session["user_id"] = user_id                                        # remember which user has logged in

        flash("Registered!")                                                # flash success message!
...
def payment():
    ...
    # makes a list of each item is from all the bids made by user
    item_ids = []
    for bid in bids:
        item_ids.append(bid["item_id"])
    ...
    # iterate through items, creating a dictionary for each to store relevant data from each field such as current bid, name and itemid for where the auction item has expired
    for item_id in items:
        tempDict = dict()
        item_status = db.execute("SELECT status FROM items WHERE item_id = :item_id", item_id = item_id)[0]["status"]
        if item_status == 0:
            ...
            tempDict["item_name"] = db.execute("SELECT item_name FROM items WHERE item_id = :item_id", item_id = item_id)[0]["item_name"]
            tempDict["item_id"] = item_id
            current_sales.append(tempDict)
            
    #if the number auctions you won is 0, it gives a invalid screen
    if len(current_sales) == 0:
        return apology("You don't have any winnings!")
    
    return render_template("payment.html", current_sales=current_sales)
...
def paymentsuccessful():
    ...
    if request.method == "POST":# if user reached route via POST (as by submitting a form via POST)
        ...
        if not request.form.get("paymenttype"):         # ensure payment type was submitted
            return apology("must Payment Type")
        
        if not request.form.get("cardnumber"):          # ensure card number was submitted
            return apology("must provide Card number")
          
        if not request.form.get("expiry"):              # ensure expiry was submitted
            return apology("must provide expiry")

        if not request.form.get("cvc"):                 # ensure cvc was submitted
            return apology("must provide cvc")

        if not request.form.get("address"):             # ensure address was submitted
            return apology("must provide address")
        
        paymenttype = request.form.get("paymenttype")   #get payment type
        cardnumber = request.form.get("cardnumber")     #get card number
        expiry = request.form.get("expiry")             #get expiry
        cvc = request.form.get("cvc")                   #get cvc
        address = request.form.get("address")           #get address
    
        session["user_id"] = user_id                    # remember which user has logged in
     
        flash("Payment successful!")                    # flash success message!
...
def sell():
    ...
    if request.method == "POST":
        ...
        try: 
            ...
            if start_bid <= 0:
                return apology("start bid must positive")
        except ValueError:
